...
- [x] **T9.5** — Make the customer's name and phone optional on bills and quotations. *(A counter sale to someone who does not give a name is ordinary, and a bill that cannot be raised is one that goes in a paper book instead. The state still blocks, because it decides the CGST/SGST split. A partly typed number is still an error — nothing is a decision, six digits is a slip. On screen a missing name shows a muted "No name", from one shared helper; the printed invoice and quotation omit the line entirely, since "No name" on a customer's copy is worse than nothing. The quotation's whole customer block disappears when empty, because unlike the invoice's it has no place of supply beneath it. A contact with a name but no number is now offered by the address-book lookup, which used to drop it.)*
- [x] **T9.6** — An optional discount per line on bills and quotations, as a percentage or a flat amount. *(Discounted, then taxed. The discount comes off the LINE rather than each unit — a flat ₹100 on three of something is ₹100, and taking a percentage per unit then multiplying multiplies its rounding error by the quantity: ₹333.33 × 3 with 10% off is 1,061.99 per line and 1,062.00 per unit. Stored on the line: what was agreed, what it actually took off, and the price basis — the last so an edit can rebuild the line rather than work backwards from a figure the discount is already out of. A discount bigger than the line makes the item free rather than a negative one, and says so. Visible on the bill and quotation screens; absent from both printed documents, which show the discounted price as the price.)*
- [x] **T9.7** — Make the date a bill or quotation was raised clearly visible on screen and on the printed documents. *(An audit found the date already on all seven surfaces — both PDFs carry a labelled Date beside the number, the Dashboard and History show it, and both detail screens have a Date row — so this is three small additions of the TIME, where two documents raised on the same day need telling apart: the quotations list and the two detail screens. History rows already had it and its sticky day heading is left alone, since the heading is what the rows beneath it have in common. Nothing is added to the PDFs: a bill is identified by its number, and the hour would only invite questions about when the shop was open. The suite that came with it is the real work — dates are stored as UTC and read in local time, so the two disagree about which day it is for five and a half hours of every day, and a bill raised at 00:15 is stored under the previous date. It pins the midnight, month, year and leap-day boundaries, the same-instant tie-break, an invalid date rendering as nothing rather than "Invalid Date", and that an edit never moves the date — which would refile the sale in another GST return period.)*
